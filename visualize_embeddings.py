...
def visualize_z1_embeddings(args):
    ...
    for i, z1_name in enumerate(z1_subtypes):
        ...
        ax1.scatter(emb[0], emb[1], s=300, c=colors[i], marker=markers[i], 
                   label=f'{z1_name} (ground truth)', alpha=0.7, edgecolors='black', linewidths=2)
    ...
    ax1.set_title(f'Ground Truth z1 Embeddings\n(z2={args.z2})', fontsize=14, fontweight='bold')
    ax1.legend(fontsize=10)

    # Right plot: Sampled embeddings
    ax2 = axes[1]
    ax2.scatter(z1_samples_np[:, 0], z1_samples_np[:, 1], 
               s=20, alpha=0.5, c='purple', label='Sampled z1')
    
    # Overlay ground truth for reference
    for i, z1_name in enumerate(z1_subtypes):
        emb = dataset.z1_embeddings[z1_name]
        ax2.scatter(emb[0], emb[1], s=300, c=colors[i], marker=markers[i], 
    ...
    ax2.set_title(f'Model B: Sampled z1 Embeddings\n(z2={args.z2}, n={args.num_samples})', 
    ...
    bars = ax.bar(labels, values, color=['red', 'blue'], alpha=0.7, edgecolor='black', linewidth=2)
    ...
    ax.axhline(expected_count, color='green', linestyle='--', linewidth=2, 
              label=f'Expected (uniform): {expected_count:.0f}')
    
    ax.set_xlabel('z1 Category', fontsize=12)
    ax.set_ylabel('Count', fontsize=12)
    ax.set_title(f'Mode Coverage Histogram\n(z2={args.z2}, n={args.num_samples})', 
                fontsize=14, fontweight='bold')
